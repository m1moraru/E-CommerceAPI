...
const usersController = {
  ...
  async loginUser(req, res) {
    try {
      ...
      // Passport will automatically set req.user and store it in the session
      req.login(user, (err) => {
        if (err) {
          return res.status(500).json({ error: 'Failed to log in.' });
        }
        // Send user data (excluding password) in the response
  ...
  async getUser(req, res) {
    try {
      const { id } = req.params;
      const user = await users.getUserById(id); // Assuming `users.getUserById` exists
  ...
  async redirectToHome(req, res) {
    try {
      if (req.isAuthenticated()) {
        const { password, ...userData } = req.user; // Remove password from user data
        return res.json({
  ...
  async getUserCreatedAt(req, res) {
    try {
      ...
      // If there's no userId, respond with an error
      if (!userId) {
        return res.status(400).json({ error: 'User ID is missing' });
      }
  
      const createdAt = await userModel.getUserCreatedAt(userId);
